...
5. 最后一步核对。默认 **Word**；**SHM-D 默认 Excel**（官方 xlsm，`.doc` 勾不了）。有载 / CMA7 也可下官方 xlsm；其余 Excel 是可读表。再下载。
...
| **干式 CZ** | Dry Type order sheet | 室内干变真空有载。三相通常 **3×CZI**。导出填官方 Word。 |
...
官方模板在 `public/templates/`。分接代码与 `oltc-selector/lib/tapCode.ts` 保持同一套图。
