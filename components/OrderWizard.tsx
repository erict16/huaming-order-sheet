...
export default function OrderWizard({ sheetId }: { sheetId: string }) {
  ...
  const showType = id !== "cma7" && id !== "shm-d" ? true : Boolean(typeStr.compact);
    ...
    <div className="mx-auto grid max-w-6xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[1fr_280px]">
      <div>
        ...
        <div className="mt-6 lg:hidden">
          <TypePlate spaced={typeStr.spaced} compact={typeStr.compact} />
        </div>
            ...
            >
              ...
              {current.kind === "review" ? (
                <div className="space-y-4">
                  ...
                  <ReviewPanel sheet={sheet} values={values} />
                </div>
              ) : null}
        ...
        <div className="no-print sticky bottom-3 z-20 mt-6 flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-card backdrop-blur">
          ...
          {step < sheet.steps.length - 1 ? (
            ...
            <button type="button" className="btn-primary" onClick={() => exportExcel(sheet, values)}>
              {chromeText("export", lang)}
            </button>
          )}
      ...
      <aside className="no-print hidden lg:block">
        <div className="sticky top-24 space-y-4">
          {showType ? <TypePlate spaced={typeStr.spaced} compact={typeStr.compact} /> : null}
          <div className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-ink-soft">
            <p className="font-semibold text-navy">{chromeText("standardNote", lang)}</p>
            <p className="mt-2 leading-relaxed">
              {sheet.id === "oltc"
                ? chromeText("selectorHint", lang)
                : sheet.id === "octc"
                  ? chromeText("octcHint", lang)
                  : sheet.id === "dry"
                    ? chromeText("dryHint", lang)
                    : sheet.id === "cma7"
                      ? chromeText("cma7Hint", lang)
                      : chromeText("shmdHint", lang)}
            </p>
            {sheet.id === "oltc" ? (
              <p className="mt-2 leading-relaxed">{chromeText("positionsHint", lang)}</p>
            ) : null}
          </div>
        </div>
      </aside>
